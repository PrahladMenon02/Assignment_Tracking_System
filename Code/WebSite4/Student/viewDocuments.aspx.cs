...
using System.Data;
using System.IO;
public partial class Student_viewDocuments : System.Web.UI.Page
{
    ...
    public void Display()
    {
        string get = "select * from tblFileupload where Student='" + Session["ID"].ToString() + "'";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (FileUpload1.HasFile)
        {
            ...
            string insert = "update tblFileupload set StudentWork=@StudentWork,Status=@Status where AssignmentID=@AssignmentID";
            cmd = new SqlCommand(insert, con);
            cmd.Parameters.AddWithValue("@StudentWork", "~/AdminuploadFiles/" + FileName);
            cmd.Parameters.AddWithValue("@Status", Label3.Text);
            cmd.Parameters.AddWithValue("@AssignmentID", Label1.Text);
            cmd.ExecuteScalar();
            Response.Write("<script>alert('Course Work Uploaded')</script>");
    ...
    protected void GridTweets_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Uploadcoursework")
        {
            x = Convert.ToInt32(e.CommandArgument);
            cmd = new SqlCommand("select * from tblFileupload where AssignmentID=@FileID", con);
            cmd.Parameters.AddWithValue("@FileID", x);
            ...
            while (dr.Read())
            {
                Label1.Text = (dr["TeacherGuidlines"].ToString());
